...
uint64_t to_int(double &d) { return *reinterpret_cast<uint64_t *>(&d); }

uint32_t to_int(float &f) { return *reinterpret_cast<uint32_t *>(&f); }
...
void print(double d) {
  ...
  printf("  Mantissa:       %lf\n", mant);
  printf("  Mantissa bits:  0x0%lx\n", static_cast<unsigned long>(mant_bin));
  printf("\n");
}
