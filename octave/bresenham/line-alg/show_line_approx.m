...
%   p1 (2x1 floats): location in smaller image for line start
%   p2 (2x1 floats): location in larger image for line end
%
% Output:
...
function I = show_line_approx(N, mult, p1, p2)
  ...
  smaller = BresenhamPlotLine(zeros(N), p1(1), p1(2), p2(1), p2(2));
  smaller2 = BresenhamPlotLine_Conservative(zeros(N), p1(1), p1(2), p2(1), p2(2));

  % populate the larger one
  q1 = floor((mult+1)/2 + (p1-1)*mult);
  q2 = floor((mult+1)/2 + (p2-1)*mult);
  larger = BresenhamPlotLine(zeros(N*mult), q1(1), q1(2), q2(1), q2(2));
