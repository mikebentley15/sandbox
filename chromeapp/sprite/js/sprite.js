...
var Sprite = function(options) {
  ...
  self.annoying = new Howl({
    src: ['./sounds/phone-sound-minimal.mp3']
  });
  self.annoying.stereo(-1);
  self.annoytime = 2500; // milliseconds
...
Sprite.prototype = {
  ...
  play: function(key) {
    ...
    var annoyIntervalId =
      window.setInterval(self.playannoy.bind(self), self.annoytime, id);
    ...
    self.sound.once('end', function() {
      ...
      window.clearInterval(annoyIntervalId);
    }, id);
  },
  
  playannoy: function(soundid) {
    var self = this;

    // lower volume on other non-annoying sound
    var fadetime = 200;
    self.sound.fade(1.0, 0.2, fadetime, soundid);
    var id;
    self.sound.once('fade', function() {
      id = self.annoying.play();
      self.annoying.pause(id);
      var current_stereo = self.annoying.stereo(null, id);
      self.annoying.stereo(-current_stereo, id);
      self.annoying.play(id);
    });
    self.annoying.once('end', function() {
      self.sound.fade(0.2, 1.0, fadetime, soundid);
    }, id);
