function update(y,m) {
  ...
  e_labels.sort(function(a,b) { return a.index - b.index; });
  e_chords.sort(function(a,b) { return a.index - b.index; });
  i_labels.sort(function(a,b) { return a.index - b.index; });
  i_chords.sort(function(a,b) { return a.index - b.index; });

  // mainLabel.style("font-size",innerRadius *.05);
  var datakey = function(d) { return d.label; };

  eText   = eGroup.selectAll("text").data(e_labels, datakey);
  iText   = iGroup.selectAll("text").data(i_labels, datakey);
  eChords = eGroup.selectAll("path").data(e_chords, datakey);
  iChords = iGroup.selectAll("path").data(i_chords, datakey);
  ...
  dGroup.transition()
      ...
      .text(td)
      .attr("transform",
            "translate(" + (outerRadius - (td.length * fs/2)/2)
                         + ","  + (outerRadius*1.1) +")"
      )
